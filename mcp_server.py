...
class SuperProductivityMCPServer:
    ...
    def setup_tools(self):
        ...
        async def handle_list_tools() -> List[types.Tool]:
            ...
            return [
                ...
                types.Tool(
                    name="delete_tag",
                    description="Delete a tag (Not fully supported by plugin API, but exposed for future use)",
                    inputSchema={
                        "type": "object",
                        "properties": {
                            "tag_id": {
    ...
    async def update_tag(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "theme_primary_color" in args:
            # We need to construct the theme object structure
            # Note: This might overwrite other theme settings if the plugin blindly applies it
            # But the plugin maps 'tagDataToPartialTagCopy' which handles basic props.
            # To update theme deeply, we might need to send the whole theme object?
            # Let's try sending it as part of 'theme' key if the plugin supports it.
            # Based on plugin.js, it calls 'tagDataToPartialTagCopy'.
            # In 'plugin-api-mapper.ts' (not visible here but assumed), it likely maps 'theme'.
            # Let's try passing 'theme' with the primary color.
            updates["theme"] = {
